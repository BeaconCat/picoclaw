...
const (
	...
	perplexityTimeout = 30 * time.Second // Perplexity, Baidu Search (LLM-based, slower)
...
func (p *BaiduSearchProvider) Search(ctx context.Context, query string, count int) (string, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return "", fmt.Errorf("Baidu Search API error (status %d): %s", resp.StatusCode, string(body))
	}
	...
	results := searchResp.References
	if len(results) == 0 {
		return fmt.Sprintf("No results for: %s", query), nil
	}

	var lines []string
	lines = append(lines, fmt.Sprintf("Results for: %s (via Baidu Search)", query))
	for i, item := range results {
		if i >= count {
			break
		}
